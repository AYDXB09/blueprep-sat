...
- **Confirmed real bug in `download-questions.js`**: `normalize()` does `Array.isArray(detail.correct_answer) ? detail.correct_answer[0] : detail.correct_answer` — discards every accepted answer form past the first. Verified live against the real API: one sampled question has 3 valid forms (`.1764`, `.1765`, `3/17`); only `.1764` gets kept. **This is an active, unfixed correctness bug** — independent of the V2 rebuild, worth patching on its own.
