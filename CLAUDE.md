...
- **Settings persists and applies, but only theme/font_size have a real visual effect wired end-to-end.** Remaining columns:
  ...
  - `mistake_resurface_days` — no consumer yet, design not finalized. A pure day-based cooldown ("don't resurface a missed question for N days") is too simple on its own — needs to also account for how fast a student is exhausting the available question pool for their weak domain/skill (a student burning through questions quickly could hit "no eligible questions left" if resurfacing is purely time-gated). Needs a real design pass — likely something that resurfaces on whichever comes first (day threshold OR pool nearing exhaustion for that scope), not just `mistake_resurface_days` read in isolation. Not built.
