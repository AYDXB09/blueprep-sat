...
- **Break interval (10 min) between R&W and Math** — recalled from general knowledge, never verified against an official source. Flagged directly in the break-screen copy, not just here.
...
- **Settings persists and applies, but only theme/font_size have a real visual effect wired end-to-end.** Other columns (`explanation_verbosity`, `mistake_resurface_days`, `show_ai_cues_default`, etc.) save correctly to `user_settings` but nothing downstream (Player, Builder) reads them yet to actually change behavior — same "persisted ≠ applied" gap that theme/font_size had until it was found and fixed. Worth checking each column has a real consumer before assuming a Settings toggle does anything beyond storing a value.
