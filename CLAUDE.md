...
- **Settings persists and applies, but only theme/font_size have a real visual effect wired end-to-end.** Remaining columns:
  ...
  - `mistake_resurface_days` — **built.** `selectQuestionIds()` in `practiceSessions.ts` now buckets the matching pool by domain+difficulty; a question whose latest attempt by this user was wrong is held out of selection until its own bucket's fresh (unseen/corrected) supply is exhausted, or until `mistake_resurface_days` days have passed since the miss (fallback ceiling, default 14, read from `user_settings` — see `getLatestAttemptStatusForUser`). Wired into Practice Builder's ad-hoc session start (`PracticeBuilder.tsx` now loads `mistake_resurface_days` on mount and passes `resurfaceForUserId`/`mistakeResurfaceDays` into every `buildFilters()` call). Full Test Setup doesn't call `selectQuestionIds` (its adaptive-module routing is separate, still using the placeholder `tier_difficulty_profiles` weights per the adaptive-threshold note above) — this only covers ad-hoc practice sessions so far.
