...
- **Full Test's adaptive routing threshold — cannot be meaningfully calibrated yet.** Checked live usage data (2026-08-07): only 197 `question_attempts` total, from **1 user**, across 179 questions. Real calibration needs population-level pass-rate data per question (item difficulty) and per-student ability distributions at each routing point — that requires many students' worth of attempts, which doesn't exist yet. `tier_difficulty_profiles` (module1: 30/45/25 easy/med/hard, tier1: 15/45/40, tier2: 40/45/15) stays as the first-pass placeholder guess until there's real multi-user data to compute against. Revisit once usage grows.
...
- **Settings persists and applies, but only theme/font_size have a real visual effect wired end-to-end.** Remaining columns:
  - `explanation_verbosity` (Brief/Detailed) — no consumer yet. Plan: the cue/choice-explanation panel in `Player.tsx` should read this and either show the short `cues.short_label`/`explanation_title` (Brief) or the full `explanation` + rationale panel (Detailed) instead of always rendering the full version. Not built.
  - `mistake_resurface_days` — no consumer yet. Meaning: how many days a missed question should be held back before it's eligible to reappear in a future practice/ad-hoc session (spaced-repetition-style cooldown), so a student doesn't immediately see the same missed question again. Currently `getMistakes()` in `practiceSessions.ts` only looks at "most recent attempt was wrong," with no time-based filtering — this setting isn't consulted anywhere yet. Not built.
